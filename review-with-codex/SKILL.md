...
```
codex exec -c 'sandbox_permissions=["disk-full-read-access"]' -c 'service_tier="fast"' "Read /tmp/plan-review.md carefully. Do NOT modify any files. Do NOT create any files. Do NOT write any code. Your only job is to review this plan and return feedback. Review the plan for: gaps in logic, missing edge cases, potential risks, unclear requirements, better approaches, and any improvements. Return an upgraded version of the plan as structured markdown to stdout."
```
